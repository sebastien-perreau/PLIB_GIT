...
//            memset(&pink_lady_manager_tab[var->spi_id][i], 0, sizeof(pink_lady_manager_params_t));
        ...
        dma_set_transfer(var->dma_id, &var->dma_params, true);      // Do not take care of the boolean value because the DMA channel is configure to execute a transfer on event when Tx is ready (IRQ source is Tx of a peripheral - see notes of dma_set_transfer()).
